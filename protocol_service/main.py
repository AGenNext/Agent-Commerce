...
app = FastAPI(
    ...
    description="Deployable validator service for UCP, AP2, A2A, and x402 Agent-Commerce protocol payloads.",
    version="0.1.0",
)
...
def profile() -> JSONResponse:
    if not PROFILE_DOC.exists():
        raise HTTPException(status_code=404, detail="Protocol profile document not found")
    return JSONResponse({"content": load_profile_doc()})


@app.get("/schema")
def schema() -> dict[str, Any]:
    if not PROFILE_SCHEMA.exists():
        raise HTTPException(status_code=404, detail="Protocol schema not found")
    return load_schema()
...
def validate_protocol(request: ValidationRequest) -> ValidationResponse:
    ...
    return ValidationResponse(
        valid=not errors,
        schema=schema_data.get("$id", "foundation-agent-commerce-protocol-profile"),
